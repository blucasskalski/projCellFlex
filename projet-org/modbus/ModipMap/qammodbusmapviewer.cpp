	...
	, m_modbusMap( 0 )
{
	...
	m_tables << newItem("Coils", false, Qt::blue ) ;				// row 0
	m_tables << newItem("Discrete Inputs", false, Qt::blue ) ;		// row 1
	m_tables << newItem("Input Registers", false, Qt::blue ) ;		// row 2
	m_tables << newItem("Holding Registers", false, Qt::blue ) ;	// row 3
	...
	for ( int i = 0 ; i < m_model->rowCount() ; ++i ) {
		for ( int j = 1 ; j < m_model->columnCount() ; ++j ) {
			QStandardItem* item = newItem("", false, Qt::blue ) ;
...
void QamModbusMapViewer::updateModbusData(int table, const QString& name )
{
	...
	QStandardItem* item1 = m_model->item( row ) ;
...
void QamModbusMapViewer::setModbusMap(QamModbusMap* map )
{
	...
	// tout replier...
	this->collapseAll() ;
	// ... et ne montrer seulement la liste des 'numbers' utilisés
...
void QamModbusMapViewer::setModbusTable(QamModbusMap::PrimaryTable table, quint16 baseNumber, int treeRow, bool isWords )
{
	...
	QStringList	list = m_modbusMap->nameList( table ) ;
	...
	for ( int i = 0 ; i < list.size() ; ++i ) {
		...
		if ( !list2.isEmpty() ) {
			for ( int j = 0 ; j < list2.size() ; ++j ) {
				...
				row << newItem("") ;
				row << newItem("") ;
				row << newItem( QString("%1").arg(mask, 4, 16, QLatin1Char('0') ).toUpper(), false, Qt::green ) ;
				row << newItem( list2.at(j), false, Qt::green ) ;
				row << newItem( m_modbusMap->data( table, list.at(i) ).comment( list2.at(j) ), false, Qt::green ) ;
				row << newItem( QString("%1").arg(val, 4, 16, QLatin1Char('0') ).toUpper(), false, Qt::green ) ;
				row << newItem( m_modbusMap->data( table, list.at(i) ).display( list2.at(j) ), false, Qt::green ) ;
				row << newItem( m_modbusMap->data( table, list.at(i) ).valueAsString( list2.at(j) ), valueEditable, Qt::green ) ;
